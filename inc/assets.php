<?php
/**
 * Contains functions for working with assets.
 *
 ...
 * @param boolean $dir            Optional. Whether to return the directory path or the theme URL path. Defaults to false (returns URL).
 */
function get_entry_dir_path( string $dir_entry_name, bool $dir = false ): string {
	// The relative path from the theme root.
	$asset_build_dir = "/build/{$dir_entry_name}/";
	// Set the absolute file path from the root directory.
	$asset_dir_path = CREATE_WORDPRESS_THEME_DIR . $asset_build_dir;

	if ( validate_path( $asset_dir_path ) ) {
		// Negotiate the base path.
		return $dir
			? $asset_dir_path
			: get_template_directory_uri() . $asset_build_dir;
	}
...
/**
 * Load the php index files from the build directory.
 */
